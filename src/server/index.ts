...
async function addServerTool({
  ...
  server.tool(toolKey, toolDescription, toolParametersSchema, async args => {
    try {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Output: ${result.output ? JSON.stringify(result.output) : 'No output'}`,
          },
        ],
      };
    } catch (error: unknown) {
...
async function createMcpServer({
  ...
  for (const connection of connections) {
    try {
      ...
      console.log(
        `Processing connection: ${connection.name} (${connection.id}) with integration ID: ${integrationId}`
      );
